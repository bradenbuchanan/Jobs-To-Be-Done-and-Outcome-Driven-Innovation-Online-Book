...
As we discussed in [Chapter 9](https://claude.ai/book/014-ChapterNine), there are many ways to slice segmentation data. If the statistical validity holds up and the cluster analysis allows for it, try to align the number of your needs-based segments with the organization's existing structure.
...
Beyond your own research, you should also layer in external market evidence. If competitors are launching campaigns around the same themes, if industry analysts are highlighting similar priorities, or if regulatory shifts are creating new urgency around specific needs, it validates that your findings reflect a broader market reality rather than just your sample's idiosyncrasies.

This is where established strategic analysis frameworks can complement your JTBD research. Tools like Porter's Five Forces, which examines competitive rivalry, supplier and buyer power, the threat of substitutes, and the threat of new entrants [47] can help you assess whether a prioritized need is also a defensible strategic position. Similarly, a PESTLE analysis (political, economic, social, technological, legal, environmental factors) [48] can surface external forces that either amplify or diminish the urgency of a customer need, forces that JTBD research alone will not capture.

You do not need to run a full Porter's or PESTLE exercise for every research project. But being aware of these lenses reinforces the principle from Chapter 10: ODI identifies customer needs, but strategy requires synthesizing those needs with competitive and environmental realities.

**The Combined Effect**

When you present all three legs together, along with supporting market context, you transform the conversation. You are no longer saying "I think we should prioritize this." You are presenting a factual case: a large sample ranked it first when forced to choose, product logs confirm the behavior is real, qualitative interviews reveal the emotional stakes, and the competitive landscape is moving in the same direction. The decision becomes easier to support.
...
User stories in agile development follow a standard format [49]:
...
A practical note: outcome-focused stories work best at the epic or initiative level, where you want to preserve solution flexibility and keep teams anchored to customer progress. At the sprint level, teams will often need to decompose these into more specific, estimable stories that describe concrete implementation work. This is not a failure of the approach — it is how outcome thinking cascades into daily execution. The outcome story sets the direction. The feature stories within it are the steps. Teresa Torres's "Opportunity Solution Tree" framework offers a useful visual model for managing this hierarchy, mapping desired outcomes to the specific solutions and experiments teams pursue [50].
...
| -------------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------- |
| **User**             | As an SRE                                         | As an SRE on call                                                                       |
| **Want**             | I want a dashboard showing third-party API status | I want to see a direct correlation between external API latency and checkout conversion |
| **So that**          | so that I can monitor external dependencies       | so that I only trigger incident response when revenue is actually at risk               |
| **Implied solution** | Dashboard with API status indicators              | Open — could be dashboard, alert system, or correlation engine                          |
| **Success criteria** | Dashboard exists and shows data                   | Incident responses correlate with actual revenue impact                                 |

**Redefining "Done"**

Outcome stories also change how you define success [51]. Instead of "the feature exists and works," the definition of done becomes a measurable outcome tied to the original need statement.
...
The three principles above may seem straightforward in isolation. The challenge is applying them together, especially when real organizational dynamics (competing priorities, skeptical stakeholders, and imperfect data) are in play. The following example walks through this process end-to-end. It uses a fictional company but draws on patterns common across real engagements. It is intentionally detailed because the devil is in the details of translation work.
...
The team chose the combined framing approach for their MaxDiff question (Option 3 from [Chapter 8](https://claude.ai/book/011-ChapterEight)). Rather than asking about importance or satisfaction separately, they asked:

"When managing client engagements, which of these unmet needs would make the biggest difference to your firm if solved?"

This framing captured both dimensions in a single question. The need had to matter (importance) and not already be solved (satisfaction gap) to rank highly. As we discussed in Chapter 8, combined framing trades the ability to distinguish "unimportant" from "already satisfied" for simplicity and reduced survey fatigue. The team accepted this trade-off knowing they would need supplementary data to identify table-stakes needs.
...
| 3    | Reduce time spent chasing team members for missing time entries        | 8.1           |
...
| 14   | Minimize time spent figuring out who is available for new assignments  | 3.2           |
| 15   | Ensure all team members understand their responsibilities from day one | 2.9           |
...
| 20   | Access engagement information from anywhere without VPN hassles        | 0.5           |
...
This convergent evidence suggested Clarify was not just seeing patterns in their own customers. They were identifying a market-wide shift in priorities. It also validated that investing in this area would address genuine competitive pressure — a consideration that JTBD research alone does not surface but that strategic frameworks like Porter's Five Forces would flag as the threat of substitutes and competitive rivalry [47].
...
### The Power of Deprioritization

Just as significant was what the research told them _not_ to build. In practice, deprioritization decisions often create more organizational clarity than prioritization ones. Every team has a backlog of things they want to build. The harder question is what to stop advocating for.
...
In each case, the research did not simply say "yes" or "no" to a stakeholder's request. It reframed the conversation around customer needs, which made the decisions easier to accept even for the people whose initiatives were deprioritized.

### Operationalization: Translating Needs into Buildable Artifacts

With the top needs validated through triangulation and mapped onto existing personas, the team translated their findings into user stories and definitions of done.

For the top-ranked need, they wrote:

> **As a** partner managing multiple engagements, **I want to** be automatically alerted when an engagement shows early warning signs of trouble, before the client notices anything is wrong, **so that** I can intervene proactively rather than manage crises.

**Definition of done:** Partners receive alerts for at-risk engagements at least 48 hours before any client-reported issue, with a false positive rate below 20%.

For the time entry need, they wrote:

> **As an** engagement manager, **I want** my team's time entries to be captured with minimal manual effort and no nagging from me, **so that** I can bill accurately without damaging team relationships.

**Definition of done:** Billable time capture completeness reaches 95% within two business days of work performed, without requiring manager-initiated follow-up more than once per week.

Notice that the time entry story explicitly incorporated the emotional dimension uncovered in qualitative research — the manager's discomfort with the "babysitter" role. This shaped the definition of done: success was not just complete time entries, but complete time entries _without the nagging_.

For the design team, they provided contextual narratives alongside the stories. The at-risk engagement alert needed to work for a partner checking their phone between client meetings — not sitting at a desk with a full dashboard. The time entry solution needed to feel like a tool for professionals, not a compliance mechanism imposed from above. These contextual details, drawn directly from interview transcripts, guided design decisions in ways the ranked data alone could not.

### What Happened Next

The team shipped the first version of their engagement health alerting system three months later. Early results showed a 31% reduction in client-reported issues among firms that adopted it. Partners reported feeling less anxious about their portfolios, validating the emotional dimension the research had uncovered.

The time entry solution had a more complicated trajectory. The first iteration automated reminders — essentially digitizing the nagging. Usage data showed that while time entry completeness improved modestly, engagement managers did not feel the burden had lifted. The team went back to the qualitative insight about role identity and redesigned the approach around passive time capture suggestions rather than active reminders. The second iteration performed significantly better.

This outcome illustrates an important reality: even good research requires iteration. The research correctly identified the need and its emotional dimensions. But translating insight into a solution that actually resonated required building, measuring, learning, and adjusting. Research reduces the risk that you are solving the wrong problem. It does not eliminate the risk that your first solution to the right problem will be imperfect.
...
**User stories should describe outcomes, not features — but decomposition is necessary.** Every story the team wrote at the initiative level focused on the progress customers wanted to make, not the specific solution. This opened up solution possibilities. At the sprint level, teams decomposed these into concrete implementation stories. Both levels served their purpose.

**Deprioritization is a research output, not just prioritization.** Some of the most valuable decisions Clarify made were about what _not_ to build. The research gave stakeholders a shared evidence base for these difficult conversations, which was far more productive than debating competing anecdotes.
...
These critiques do not mean you should abandon the effort to understand and quantify customer needs. They mean you should do it more carefully. The MaxDiff approach addresses the methodological weaknesses while preserving the core insight: customers have jobs they are trying to accomplish, and your product either helps them make progress or it does not.
...
[47] Porter, Michael E. "How Competitive Forces Shape Strategy." _Harvard Business Review_, March 1979. Available at: [https://hbr.org/1979/03/how-competitive-forces-shape-strategy](https://hbr.org/1979/03/how-competitive-forces-shape-strategy)

[48] Aguilar, Francis J. _Scanning the Business Environment_. Macmillan, 1967.

[49] Cohn, M. (2004). _User Stories Applied: For Agile Software Development_. Addison-Wesley Professional. Retrieved from [https://www.amazon.com/User-Stories-Applied-Software-Development/dp/0321205685](https://www.amazon.com/User-Stories-Applied-Software-Development/dp/0321205685)

[50] Torres, Teresa. _Continuous Discovery Habits: Discover Products That Create Customer Value and Business Value_. Product Talk LLC, 2021. Retrieved from [https://www.producttalk.org/](https://www.producttalk.org/)

[51] Schwaber, K., & Sutherland, J. (2020). _The Scrum Guide_. Scrum.org. Retrieved from [https://scrumguides.org/](https://scrumguides.org/)
